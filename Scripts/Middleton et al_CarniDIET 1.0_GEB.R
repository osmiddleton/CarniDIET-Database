...
#        - Percentage of taxonomic orders making up species-level records
#       
#   A note on taxonomy:
...
# CarniDIET 1.0:
carnidiet <- read.csv("./Version 1.0/CarniDIET 1.0.csv", stringsAsFactors = TRUE)
str(carnidiet)
...
cd.wos.hits <- read.csv("./Version 1.0/Supplementary data/Potential species list.csv", header = TRUE, stringsAsFactors = TRUE)
...
phylacine <- read.csv("../Phylacine/Trait_data.csv", stringsAsFactors = TRUE) # Traits
forest <- read.nexus("../Phylacine/Phylogenies/Complete_phylogeny.nex") # Mammalian phylogeny
...
# Mammal-consumer numbers ----

# How many mammal-consumers in CarniDIET?
nlevels(carnidiet$scientificNameCarni) # 103 scientific names
...
# How many levels of life stage?
nlevels(carnidiet$lifeStageCarni) # 4 ages/life-stages

# How many levels of sex?
nlevels(carnidiet$sex) # Separated for Males and Female, or grouped for both Males and Females
...
nlevels(carnidiet$speciesPrey) # 711 prey species
nlevels(carnidiet$scientificNamePrey) # 823 prey binomials
...
range(as.numeric(carnidiet[!is.na(carnidiet$startDayOfYear),]$startDayOfYear))
range(as.numeric(carnidiet[!is.na(carnidiet$endDayOfYear),]$endDayOfYear))

# Seasons - these are pretty crude
nlevels(carnidiet$season) # 45 seasons
...
levels(carnidiet$country) # 97 countries
...
levels(carnidiet$sourceCollectionReference) # 686 Collection Refs

# Number of studies ----

# A source (e.g. a peer-reviewed paper) can consist of >1 'study'

# A study is a subset of a source that contains a diet composition(s)
# for a specific site, or for a demographic breakdown for that species.
# A study can consist of multiple diet compositions from multiple seasons or years,
# and also use multiple sampling protocols or quantification methods.

# Find diet compositions
carni.studies <- carnidiet %>% 
  dplyr::group_by(# Unique species and demography
                  familyCarni, scientificNameCarni, lifeStageCarni, sexCarni, 
...
# Identify how many diet compositions occur in individual studies, as studies
# can have diet compositions for multiple seasons or years, as well as for
# multiple quantification methods and sampling protocols.
...
# How many are single studies:
nrow(carni.studies %>% filter(seasons == 1 & years == 1)) # 990
990/1310 # 76%


# How many are seasonal comparisons:
nrow(carni.studies %>% filter(seasons > 1 & years == 1)) # 198
198/1310
...
#————————————————————————————————————————————————————————————————————————————————————————————####
# 2. Species covered by diet studies  ----
#————————————————————————————————————————————————————————————————————————————————————————————####

# In this section we look at how many of the potential mammal-consumers 
# identified (n = 210) are covered by diet studies.
...
family.papers <- carnidiet[c(3,53)]
...
family.species.papers <- carnidiet[c(3:4,53)]
...
genus.species.papers <- diet.2[c(3:5,54)]
...
# Figure S3: Scatterplot family-level diet representation ----
...
# Figure S6: Barplot for top-20 studied species in database ----

# How many sources feature each species?
...
# How many studies are for each species?
carni.studies <- carnidiet %>% 
  dplyr::group_by(# Unique species and demography
                  familyCarni, scientificNameCarni, lifeStageCarni, sexCarni,
...
sum(carnivore.summary.master$studies) # 1310 - This is all individual studies
...
(p10 <- ggplot(data = y[1:20,], aes(x = reorder(CarniBinom, studies), y = studies)) + # 20 species refers to 66% all papers
  ...
  annotate("text", x = 20, y = 175, label = "(91)", size = 2, colour = "black") +
    annotate("text", x = 19, y = 92, label = "(60)", size = 2, colour = "black") +
    ...
    annotate("text", x = 14, y = 43, label = "(21)", size = 2, colour = "black") +
    annotate("text", x = 13, y = 43, label = "(19)", size = 2, colour = "black") +
    annotate("text", x = 12, y = 40, label = "(20)", size = 2, colour = "black") +
    annotate("text", x = 11, y = 36, label = "(25)", size = 2, colour = "black") +
    annotate("text", x = 10, y = 36, label = "(22)", size = 2, colour = "black") +
    annotate("text", x = 9, y = 36, label = "(21)", size = 2, colour = "black") +
    annotate("text", x = 8, y = 34, label = "(25)", size = 2, colour = "black") +
    annotate("text", x = 7, y = 32, label = "(14)", size = 2, colour = "black") +
    annotate("text", x = 6, y = 32, label = "(20)", size = 2, colour = "black") +
    annotate("text", x = 5, y = 32, label = "(15)", size = 2, colour = "black") +
    annotate("text", x = 4, y = 31, label = "(19)", size = 2, colour = "black") +
    annotate("text", x = 3, y = 29, label = "(16)", size = 2, colour = "black") +
    annotate("text", x = 2, y = 27, label = "(15)", size = 2, colour = "black") +
    annotate("text", x = 1, y = 26, label = "(11)", size = 2, colour = "black") +
  
    # Add changes to axes  
  ylim(0,200) + theme(axis.text.y = element_text(face = "italic")))

# Figure 2a: Phylogenetic tree and number of studies per species ----
names(forest) <- NULL
set.seed(42)
...
tree <- ggtree::fortify(tree) # Previously using fortify but no longer working...
...
# Make sure colour scheme is consistent throughout for families
...
# Barplot showing studies per species and coloured by family.
# Figure goes from most (left) to least studied (right).
...
# Figure 1 added together in pre-processing using Inkscape.



# Figure S7: Repeat with all potential mammal-consumers ----
...
#————————————————————————————————————————————————————————————————————————————————————————————####
# 3. Spatial and temporal distribution of diet studies ----
#————————————————————————————————————————————————————————————————————————————————————————————####

# Where are mammal-consumers distributed? ----
# Let's check this by ecoregion
...
# Figure S4: Distributions by ecoregion ----
...
# Missing species richness of mammal-consumers by ecoregion
...
# Total species richness of mammal-consumers by ecoregion
...
# Where are studies distributed in relation to the species richness of mammal
# consumers by ecoregion?

# Get the unique study sites (but keep year information)
...
nrow(temporal.diet) # 1574 'studies' when keeping years separate
...
temporal.diet.sp <- SpatialPointsDataFrame(coords = coords,
                                  ...
                                  proj4string = crs(biomes))

# Quick check of study sites
...
p2 <- ggplot() +
  ...
  scale_fill_gradient(low = "white", high = "black", name = "Ecoregion SR") +
...
# Add the spatial and temporal information figures together...
p3 <- ggarrange(p1,
                p2,
...
# Ecoregion species richness and number of studies ----
...
# Number of studies per ecoregion
...
# Significant, but very low correlation
cor.test(ecoregions.3$SpecRich, ecoregions.3$study.count)

# Figure S5: Relationships between studies and mammal-consumer species richness ----
(cor <- ggplot(data = ecoregions.3[ecoregions.3$SpecRich < 30,], 
               aes(x = SpecRich, y = study.count)) +
...
# Diet resolution of mammals in diets

# Number of unique prey species in mammal-consumer diets
...
# Number per mammal-consumers
prey.consumed <- prey %>%
                 dplyr::group_by(scientificNameCarni) %>%
                 dplyr::summarise(Prey_species_richness = length(scientificNameCarni))
...
# Percentage of records for different taxonomic resolutions
colnames(carnidiet)
...
colnames(total.obvs) <- "Total_records" # Only considering key taxonomic ranks.
...
# Table describing species-level information of diet records:
diet.summary

# Get some descriptive statistics for these
colnames(diet.summary)

# sampling protocols used for data collection
...
# Figure 2c: Records from each sampling protocols and quantification methods ----
...
# Main text, only shows the key factor levels
...
# Get counts for each sampling protocol
sample.count <- diet.method.summary %>%
                dplyr::group_by(samplingProtocol) %>%
                dplyr::summarize(total.count = sum(count)) 
...
# ---- Figure S9: Same but with ALL records ----

# Summarise data
diet.method.summary <- carnidiet %>% dplyr::group_by(samplingProtocol, methodQuantification) %>% dplyr::summarise(count = length(methodQuantification))

# Need to show as percentages
# Get counts for each sample source
...
levels(diet.method.summary.sum$samplingProtocol)
...
(p1.1 <- ggplot(diet.method.summary, aes(x = samplingProtocol, y = Perc, fill = methodQuantification)) +
    geom_bar(stat = "identity", colour = "black", lwd = 0.1) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, vjust = .5)) +
    #scale_y_continuous(limits = c(0,18000), expand = c(0, 0)) +
...
# Figure 2b: Records from each sampling protocol and taxonomic resolution ----
...
# Figure S8: ALL RECORDS from each sampling protocol and taxonomic resolution ----
diet.resolution  <- carnidiet %>% dplyr::group_by(samplingProtocol, taxonRankPrey) %>% dplyr::summarise(count = length(taxonRankPrey))
...
# Proportion of prey orders in CarniDIET ----

# This is relative to the number of diet records in CarniDIET at the 
# species-level.

# Extract data
x <- carnidiet[carnidiet$taxonRankPrey == "Species",]
colnames(x)
prey.info <- x[c(7:13)]

# Proportion of orders in predator-prey records
t <- as.data.frame(table(prey.info$orderPrey))
...
# Some random bits of code that might be used in the future...
